package modules.tsrouteservice.route.config;
...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity httpSecurity) throws Exception {
        // close session
        // close default csrf
        httpSecurity.httpBasic().disable().csrf().disable().sessionManagement().sessionCreationPolicy(SessionCreationPolicy.STATELESS).and().authorizeRequests().antMatchers("/api/v1/routeservice/**").permitAll().antMatchers(HttpMethod.POST, "/api/v1/routeservice/routes").hasAnyRole("ADMIN").antMatchers(HttpMethod.DELETE, "/api/v1/routeservice/routes/*").hasAnyRole("ADMIN").antMatchers("/swagger-ui.html", "/webjars/**", "/images/**", "/configuration/**", "/swagger-resources/**", "/v2/**").permitAll().anyRequest().authenticated().and().addFilterBefore(new JWTFilter(), UsernamePasswordAuthenticationFilter.class);
        // close cache
