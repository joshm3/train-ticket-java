package modules.tsnotificationservice.notification.controller;
import modules.tsnotificationservice.notification.entity.NotifyInfo;
import modules.tsnotificationservice.notification.mq.RabbitSend;
import modules.tsnotificationservice.notification.service.NotificationService;
...
public class NotificationController {
    ...
    @PostMapping("/notification/preserve_success")
    public boolean preserve_success(@RequestBody
    NotifyInfo info, @RequestHeader
    HttpHeaders headers) {
        return service.preserveSuccess(info, headers);
    }

    @PostMapping("/notification/order_create_success")
    public boolean order_create_success(@RequestBody
    NotifyInfo info, @RequestHeader
    HttpHeaders headers) {
        return service.orderCreateSuccess(info, headers);
    }

    @PostMapping("/notification/order_changed_success")
    public boolean order_changed_success(@RequestBody
    NotifyInfo info, @RequestHeader
    HttpHeaders headers) {
        return service.orderChangedSuccess(info, headers);
    }

    @PostMapping("/notification/order_cancel_success")
    public boolean order_cancel_success(@RequestBody
    NotifyInfo info, @RequestHeader
    HttpHeaders headers) {
