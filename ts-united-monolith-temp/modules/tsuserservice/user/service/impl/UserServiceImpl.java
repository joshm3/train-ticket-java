package modules.tsuserservice.user.service.impl;
...
public class UserServiceImpl implements UserService {
    ...
    public Response saveUser(UserDto userDto, HttpHeaders headers) {
        ...
        User user = User.builder().userId(userId).userName(userDto.getUserName()).password(userDto.getPassword()).gender(userDto.getGender()).documentType(userDto.getDocumentType()).documentNum(userDto.getDocumentNum()).email(userDto.getEmail()).build();
        // avoid same user name
        User user1 = userRepository.findByUserName(userDto.getUserName());
        if (user1 == null) {
            createDefaultAuthUser(AuthDto.builder().userId(userId + "").userName(user.getUserName()).password(user.getPassword()).build());
    ...
    private Response createDefaultAuthUser(AuthDto dto) {
        ...
        ResponseEntity<Response<AuthDto>> res = restTemplate.exchange(auth_service_url + "/api/v1/auth", HttpMethod.POST, entity, new ParameterizedTypeReference<Response<AuthDto>>() {});
        return res.getBody();
    }
    ...
    public Response updateUser(UserDto userDto, HttpHeaders headers) {
        ...
        if (oldUser != null) {
            User newUser = User.builder().email(userDto.getEmail()).password(userDto.getPassword()).userId(oldUser.getUserId()).userName(userDto.getUserName()).gender(userDto.getGender()).documentNum(userDto.getDocumentNum()).documentType(userDto.getDocumentType()).build();
    ...
    public void deleteUserAuth(String userId, HttpHeaders headers) {
        ...
        restTemplate.exchange((AUTH_SERVICE_URI + "/users/") + userId, HttpMethod.DELETE, httpEntity, Response.class);
