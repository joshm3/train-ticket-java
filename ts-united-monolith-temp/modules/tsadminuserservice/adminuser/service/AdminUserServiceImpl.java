...
public class AdminUserServiceImpl implements AdminUserService {
    ...
    public Response getAllUsers(HttpHeaders headers) {
        ...
        ResponseEntity<Response<List<User>>> re = restTemplate.exchange(USER_SERVICE_IP_URI, HttpMethod.GET, requestEntity, new ParameterizedTypeReference<Response<List<User>>>() {});
    ...
    public Response deleteUser(String userId, HttpHeaders headers) {
        ...
        String user_service_url = getServiceUrl("ts-user-service");
        String USER_SERVICE_IP_URI = user_service_url + "/api/v1/userservice/users";
        ResponseEntity<Response> re = restTemplate.exchange((USER_SERVICE_IP_URI + "/") + userId, HttpMethod.DELETE, requestEntity, Response.class);
        if ((re.getBody() == null) || (re.getBody().getStatus() != 1)) {
    ...
    public Response updateUser(UserDto userDto, HttpHeaders headers) {
        ...
        String user_service_url = getServiceUrl("ts-user-service");
        String USER_SERVICE_IP_URI = user_service_url + "/api/v1/userservice/users";
        ResponseEntity<Response> re = restTemplate.exchange(USER_SERVICE_IP_URI, HttpMethod.PUT, requestEntity, Response.class);
    ...
    public Response addUser(UserDto userDto, HttpHeaders headers) {
        ...
        String user_service_url = getServiceUrl("ts-user-service");
        String USER_SERVICE_IP_URI = user_service_url + "/api/v1/userservice/users";
        ResponseEntity<Response<User>> re = restTemplate.exchange(USER_SERVICE_IP_URI + "/register", HttpMethod.POST, requestEntity, new ParameterizedTypeReference<Response<User>>() {});
        String userName = userDto.getUserName();
        if ((re.getBody() == null) || (re.getBody().getStatus() != 1)) {
