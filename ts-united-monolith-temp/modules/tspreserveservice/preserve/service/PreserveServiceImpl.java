...
import modules.tspreserveservice.preserve.mq.RabbitSend;
...
public class PreserveServiceImpl implements PreserveService {
    ...
    public Response preserve(OrderTicketsInfo oti, HttpHeaders headers) {
        ...
        if (oti.getSeatType() == SeatClass.FIRSTCLASS.getCode()) {
            ...
            Ticket ticket = dipatchSeat(oti.getDate(), order.getTrainNumber(), fromStationName, toStationName, SeatClass.FIRSTCLASS.getCode(), firstClassTotalNum, stationList, headers);
            ...
            Ticket ticket = dipatchSeat(oti.getDate(), order.getTrainNumber(), fromStationName, toStationName, SeatClass.SECONDCLASS.getCode(), secondClassTotalNum, stationList, headers);
        ...
        if (oti.getAssurance() == 0) {
            ...
            Response addAssuranceResult = addAssuranceForOrder(oti.getAssurance(), cor.getData().getId().toString(), headers);
    ...
    private Response addAssuranceForOrder(int assuranceType, String orderId, HttpHeaders httpHeaders) {
        ...
        ResponseEntity<Response> reAddAssuranceResult = restTemplate.exchange((((assurance_service_url + "/api/v1/assuranceservice/assurances/") + assuranceType) + "/") + orderId, HttpMethod.GET, requestAddAssuranceResult, Response.class);
    ...
    private Response<TripAllDetail> getTripAllDetailInformation(TripAllDetailInfo gtdi, HttpHeaders httpHeaders) {
        ...
        ResponseEntity<Response<TripAllDetail>> reGetTripAllDetailResult = restTemplate.exchange(travel_service_url + "/api/v1/travelservice/trip_detail", HttpMethod.POST, requestGetTripAllDetailResult, new ParameterizedTypeReference<Response<TripAllDetail>>() {});
    ...
    private Response createOrder(Order coi, HttpHeaders httpHeaders) {
        ...
        ResponseEntity<Response<Order>> reCreateOrderResult = restTemplate.exchange(order_service_url + "/api/v1/orderservice/order", HttpMethod.POST, requestEntityCreateOrderResult, new ParameterizedTypeReference<Response<Order>>() {});
