package modules.tstravel2service.travel2.config;
...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity httpSecurity) throws Exception {
        // close session
        // close default csrf
        httpSecurity.httpBasic().disable().csrf().disable().sessionManagement().sessionCreationPolicy(SessionCreationPolicy.STATELESS).and().authorizeRequests().antMatchers("/api/v1/travel2service/**").permitAll().antMatchers(HttpMethod.PUT, trips).hasAnyRole(admin).antMatchers(HttpMethod.POST, trips).hasAnyRole(admin).antMatchers(HttpMethod.DELETE, trips).hasAnyRole(admin).antMatchers("/swagger-ui.html", "/webjars/**", "/images/**", "/configuration/**", "/swagger-resources/**", "/v2/**").permitAll().anyRequest().authenticated().and().addFilterBefore(new JWTFilter(), UsernamePasswordAuthenticationFilter.class);
        // close cache
