package modules.tswaitorderservice.waitorder.service.Impl;
import java.util.*;
...
import modules.tswaitorderservice.waitorder.repository.WaitListOrderRepository;
import modules.tswaitorderservice.waitorder.service.WaitListOrderService;
import modules.tswaitorderservice.waitorder.utils.PollThread;
...
public class WaitListOrderServiceImpl implements WaitListOrderService {
    ...
    public Response getAllWaitListOrders(HttpHeaders headers) {
        ...
        if ((orderList != null) && (!orderList.isEmpty())) {
            ...
            orderList = orderList.stream().filter(WaitListOrder -> filterList.contains(WaitListOrder.getStatus())).collect(Collectors.toList());
            return new Response(1, "Success.", orderList);
        } else {
    ...
    private Boolean WaitListOrderExist(List<WaitListOrder> orderList, WaitListOrderVO newOrder) {
        for (WaitListOrder order : orderList) {
            if (((((Objects.equals(order.getAccountId(), newOrder.getAccountId()) && Objects.equals(order.getContactsId(), newOrder.getContactsId())) && Objects.equals(order.getTrainNumber(), newOrder.getTripId())) && Objects.equals(order.getTravelTime(), newOrder.getDate())) && Objects.equals(order.getFrom(), newOrder.getFrom())) && Objects.equals(order.getTo(), newOrder.getTo())) {
                return true;
            }
        }
        return false;
