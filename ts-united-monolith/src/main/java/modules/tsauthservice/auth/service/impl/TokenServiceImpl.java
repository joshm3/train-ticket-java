...
import modules.tsauthservice.TsauthserviceClient;
...
public class TokenServiceImpl implements TokenService {
    @Autowired
    TsauthserviceClient tsauthserviceClient;
    ...
    public Response getToken(BasicAuthDto dto, HttpHeaders headers) throws UserOperationException {
        ...
        if (!StringUtils.isEmpty(verifyCode)) {
            ...
            ResponseEntity<Boolean> re = tsauthserviceClient.getApiV1VerifycodeVerifyPathvariable(dto.getVerificationCode(), headers);
        ...
        UserFromtsauthservice user = userRepository.findByUsername(username).orElseThrow(() -> new UserOperationException(MessageFormat.format(InfoConstant.USER_NAME_NOT_FOUND_1, username)));
