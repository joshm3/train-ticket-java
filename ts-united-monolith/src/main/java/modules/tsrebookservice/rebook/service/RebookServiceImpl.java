...
import modules.tsrebookservice.TsrebookserviceClient;
...
public class RebookServiceImpl implements RebookService {
    @Autowired
    TsrebookserviceClient tsrebookserviceClient;
    ...
    private Response updateOrder(Order order, RebookInfo info, TripAllDetail gtdr, String ticketPrice, HttpHeaders httpHeaders) {
        ...
        if (info.getSeatType() == SeatClass.FIRSTCLASS.getCode()) {
            // Dispatch the seat
            Ticket ticket = dipatchSeat(info.getDate(), order.getTrainNumber(), order.getFrom(), order.getTo(), SeatClass.FIRSTCLASS.getCode(), firstClassTotalNum, stations, httpHeaders);
            order.setSeatClass(SeatClass.FIRSTCLASS.getCode());
            order.setSeatNumber("" + ticket.getSeatNo());
        } else {
            Ticket ticket = dipatchSeat(info.getDate(), order.getTrainNumber(), order.getFrom(), order.getTo(), SeatClass.SECONDCLASS.getCode(), secondClassTotalNum, stations, httpHeaders);
    ...
    public Ticket dipatchSeat(String date, String tripId, String startStationId, String endStataionId, int seatType, int tatalNum, List<String> stations, HttpHeaders httpHeaders) {
        ...
        ResponseEntity<Response<Ticket>> reTicket = tsrebookserviceClient.postApiV1SeatserviceSeats(seatRequest, newHeaders);
    ...
    private Response<TripAllDetail> getTripAllDetailInformation(TripAllDetailInfo gtdi, String tripId, HttpHeaders httpHeaders) {
        ...
        ResponseEntity<Response<TripAllDetail>> reGetTripAllDetailResult = null;
        if (tripId.startsWith("G") || tripId.startsWith("D")) {
            requestUrl = travel_service_url + "/api/v1/travelservice/trip_detail";
            reGetTripAllDetailResult = tsrebookserviceClient.postApiV1TravelserviceTrip_detail(gtdi, newHeaders);
            // ts-travel-service:12346/travel/getTripAllDetailInfo
        } else {
            requestUrl = travel2_service_url + "/api/v1/travel2service/trip_detail";
            reGetTripAllDetailResult = tsrebookserviceClient.postApiV1Travel2serviceTrip_detail(gtdi, newHeaders);
    ...
    private Response createOrder(Order order, String tripId, HttpHeaders httpHeaders) {
        ...
        ResponseEntity<Response> reCreateOrder = null;
        if (tripId.startsWith("G") || tripId.startsWith("D")) {
            // ts-order-service:12031/order/create
            requestUrl = order_service_url + "/api/v1/orderservice/order";
            reCreateOrder = tsrebookserviceClient.postApiV1OrderserviceOrder(order, newHeaders);
        } else {
            // ts-order-other-service:12032/orderOther/create
            requestUrl = order_other_service_url + "/api/v1/orderOtherService/orderOther";
            reCreateOrder = tsrebookserviceClient.postApiV1OrderotherserviceOrderother(order, newHeaders);
        }
    ...
    private Response updateOrder(Order info, String tripId, HttpHeaders httpHeaders) {
        ...
        ResponseEntity<Response> reUpdateOrder = null;
        if (tripGD(tripId)) {
            requestOrderUtl = order_service_url + "/api/v1/orderservice/order";
            reUpdateOrder = tsrebookserviceClient.putApiV1OrderserviceOrder(info, newHeaders);
        } else {
            requestOrderUtl = order_other_service_url + "/api/v1/orderOtherService/orderOther";
            reUpdateOrder = tsrebookserviceClient.putApiV1OrderotherserviceOrderother(info, newHeaders);
        }
    ...
    private Response deleteOrder(String orderId, String tripId, HttpHeaders httpHeaders) {
        ...
        ResponseEntity<Response> reDeleteOrder = null;
        if (tripGD(tripId)) {
            requestUrl = (order_service_url + "/api/v1/orderservice/order/") + orderId;
            reDeleteOrder = restTemplate.exchange(requestUrl, HttpMethod.POST, requestDeleteOrder, Response.class);
        } else {
    ...
    private Response<Order> getOrderByRebookInfo(RebookInfo info, HttpHeaders httpHeaders) {
        ...
        if (info.getOldTripId().startsWith("G") || info.getOldTripId().startsWith("D")) {
            ...
            reGetOrderByRebookInfo = tsrebookserviceClient.getApiV1OrderserviceOrderPathvariable(info.getOrderId(), newHeaders);
        } else {
            requestUrl = (order_other_service_url + "/api/v1/orderOtherService/orderOther/") + info.getOrderId();
            reGetOrderByRebookInfo = tsrebookserviceClient.getApiV1OrderotherserviceOrderotherPathvariable(info.getOrderId(), newHeaders);
        }
    ...
    public TrainType queryTrainTypeByName(String trainTypeName, HttpHeaders headers) {
        ...
        ResponseEntity<Response> re = tsrebookserviceClient.getApiV1TrainserviceTrainsBynamePathvariable(trainTypeName, null);
    ...
    private Route getRouteByRouteId(String routeId, HttpHeaders headers) {
        ...
        ResponseEntity<Response> re = tsrebookserviceClient.getApiV1RouteserviceRoutesPathvariable(routeId, null);
    ...
    private boolean payDifferentMoney(String orderId, String tripId, String userId, String money, HttpHeaders httpHeaders) {
        ...
        ResponseEntity<Response> rePayDifferentMoney = tsrebookserviceClient.postApiV1Inside_pay_serviceInside_paymentDifference(info, newHeaders);
    ...
    private boolean drawBackMoney(String userId, String money, HttpHeaders httpHeaders) {
        ...
        String inside_payment_service_url = getServiceUrl("ts-inside-payment-service");
        ResponseEntity<Response> reDrawBackMoney = tsrebookserviceClient.getApiV1Inside_pay_serviceInside_paymentDrawbackPathvariablePathvariable(userId, money, newHeaders);
