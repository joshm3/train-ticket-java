...
import modules.tscommon.edu.fudan.common.entity.SoldTicket;
...
import modules.tsorderotherservice.TsorderotherserviceClient;
...
import modules.tsorderotherservice.other.repository.OrderOtherRepository;
...
public class OrderOtherServiceImpl implements OrderOtherService {
    @Autowired
    TsorderotherserviceClient tsorderotherserviceClient;
    ...
    public Response getSoldTickets(Seat seatRequest, HttpHeaders headers) {
        ArrayList<OrderFromtsorderotherservice> list = orderOtherRepository.findByTravelDateAndTrainNumber(seatRequest.getTravelDate(), seatRequest.getTrainNumber());
        if ((list != null) && (!list.isEmpty())) {
            Set ticketSet = new HashSet();
            for (OrderFromtsorderotherservice tempOrder : list) {
    ...
    public Response findOrderById(String id, HttpHeaders headers) {
        Optional<OrderFromtsorderotherservice> op = orderOtherRepository.findById(id);
        if (!op.isPresent()) {
            ...
            OrderFromtsorderotherservice order = op.get();
    ...
    @Override
    public Response create(OrderFromtsorderotherservice order, HttpHeaders headers) {
        OrderOtherServiceImpl.LOGGER.info("[create][Create Order][Ready Create Order]");
        ArrayList<OrderFromtsorderotherservice> accountOrders = orderOtherRepository.findByAccountId(order.getAccountId());
    ...
    @Override
    public void initOrder(OrderFromtsorderotherservice order, HttpHeaders headers) {
        Optional<OrderFromtsorderotherservice> op = orderOtherRepository.findById(order.getId());
        if (!op.isPresent()) {
            orderOtherRepository.save(order);
            OrderOtherServiceImpl.LOGGER.info("[initOrder][Init Order Success][OrderId: {}]", order.getId());
        } else {
            OrderFromtsorderotherservice orderTemp = op.get();
    ...
    public Response alterOrder(OrderAlterInfo oai, HttpHeaders headers) {
        ...
        OrderFromtsorderotherservice oldOrder = orderOtherRepository.findById(oldOrderId).get();
        oldOrder.setStatus(OrderStatus.CANCEL.getCode());
        saveChanges(oldOrder, headers);
        OrderFromtsorderotherservice newOrder = oai.getNewOrderInfo();
    ...
    @Override
    public Response<ArrayList<OrderFromtsorderotherservice>> queryOrders(QueryInfo qi, String accountId, HttpHeaders headers) {
        // 1.Get all orders of the user
        ArrayList<OrderFromtsorderotherservice> list = orderOtherRepository.findByAccountId(accountId);
        ...
        if ((qi.isEnableStateQuery() || qi.isEnableBoughtDateQuery()) || qi.isEnableTravelDateQuery()) {
            ArrayList<OrderFromtsorderotherservice> finalList = new ArrayList<>();
            for (OrderFromtsorderotherservice tempOrder : list) {
    ...
    public Response queryOrdersForRefresh(QueryInfo qi, String accountId, HttpHeaders headers) {
        ArrayList<OrderFromtsorderotherservice> orders = queryOrders(qi, accountId, headers).getData();
        ArrayList<String> stationIds = new ArrayList<>();
        for (OrderFromtsorderotherservice order : orders) {
    ...
    public List<String> queryForStationId(List<String> ids, HttpHeaders headers) {
        ...
        ResponseEntity<Response<List<String>>> re = tsorderotherserviceClient.postApiV1StationserviceStationsNamelist(ids, null);
        OrderOtherServiceImpl.LOGGER.info("[queryForStationId][Stations name list][Name List is : {}]", re.getBody().toString());
        return re.getBody().getData();
    }

    @Override
    public Response saveChanges(OrderFromtsorderotherservice order, HttpHeaders headers) {
        Optional<OrderFromtsorderotherservice> op = orderOtherRepository.findById(order.getId());
        if (!op.isPresent()) {
            ...
            OrderFromtsorderotherservice oldOrder = op.get();
    ...
    public Response cancelOrder(String accountId, String orderId, HttpHeaders headers) {
        Optional<OrderFromtsorderotherservice> op = orderOtherRepository.findById(orderId);
        if (!op.isPresent()) {
    ...
    public Response queryAlreadySoldOrders(Date travelDate, String trainNumber, HttpHeaders headers) {
        ArrayList<OrderFromtsorderotherservice> orders = orderOtherRepository.findByTravelDateAndTrainNumber(StringUtils.Date2String(travelDate), trainNumber);
    ...
    public Response getAllOrders(HttpHeaders headers) {
        ArrayList<OrderFromtsorderotherservice> orders = orderOtherRepository.findAll();
        if (orders == null) {
    ...
    public Response checkSecurityAboutOrder(Date dateFrom, String accountId, HttpHeaders headers) {
        ...
        ArrayList<OrderFromtsorderotherservice> orders = orderOtherRepository.findByAccountId(accountId);
    ...
    public Response deleteOrder(String orderId, HttpHeaders headers) {
        ...
        Optional<OrderFromtsorderotherservice> op = orderOtherRepository.findById(orderUuid);
        if (!op.isPresent()) {
    ...
    @Override
    public Response addNewOrder(OrderFromtsorderotherservice order, HttpHeaders headers) {
    ...
    @Override
    public Response updateOrder(OrderFromtsorderotherservice order, HttpHeaders headers) {
